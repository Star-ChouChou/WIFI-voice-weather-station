...
typedef struct
{
	vu16 LCD_REG;  //0x6C0007FE  0X7FE  HADDR 第十一位 为0  写入命令
	vu16 LCD_RAM;  //0x6C000800  0X800  HADDR 第十一位 为1  写入数据
} LCD_TypeDef;
...
//////////////////////////////////////////////////////////////////////////////////
//0111 1111 1110    0X7FE  第十一位 为0  
//1000 0000 0000    0X800  第十一位 为1  
//0x6C0007FE
...
//画笔颜色  RGB565  表示一个像素点颜色时  需要16bit  红色5bit 绿色6bit 蓝色5bit 1111 1000 0000 0000 
#define WHITE         	 0xFFFF
#define BLACK         	 0x0000
#define BLUE         	 0x001F
#define BRED             0XF81F
...
void TFTLCD_Init(void);	//屏幕初始化												   	//初始化
void LCD_DisplayOn(void);//显示开启										//开显示
void LCD_DisplayOff(void);	//关闭显示										//关显示
void LCD_Clear(uint16_t Color);	//清屏 												//清屏
void LCD_SetCursor(uint16_t Xpos, uint16_t Ypos);										//设置光标
void LCD_SetShowArea(uint16_t begine_x,uint16_t end_x,uint16_t begine_y,uint16_t end_y);
void LCD_DrawPoint(uint16_t x,uint16_t y);//画点										//画点
void LCD_Fast_DrawPoint(uint16_t x,uint16_t y,uint16_t color);//快速画点								//快速画点
