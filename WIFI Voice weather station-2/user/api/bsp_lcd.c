...
#include "delay.h"
//LCD的画笔颜色和背景色
uint16_t POINT_COLOR=0x0000;	//画笔颜色 黑色
uint16_t BACK_COLOR=0xFFFF;     //背景色  白色
...
{
  if(lcddev.id==0X9341||lcddev.id==0X9486||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X1963)
...
//  printf(" TFTLCD ID:%x\r\n",lcddev.id); //打印LCD ID
  if(lcddev.id==0X9341)	//9341初始化
  {
	...
	else if (lcddev.id==0x9486)//320 * 480  
	{
		LCD_WR_REG(0XF2);
		LCD_WR_DATA(0x18);
		LCD_WR_DATA(0xA3);
...
void LCD_Draw_Circle(uint16_t x0,uint16_t y0,uint8_t r)
{
  ...
  while(a<=b)
  {
    LCD_Fast_DrawPoint(x0+a,y0-b,BLACK);             //5
    LCD_Fast_DrawPoint(x0+b,y0-a,BLACK);             //0
    LCD_Fast_DrawPoint(x0+b,y0+a,BLACK);             //4
    LCD_Fast_DrawPoint(x0+a,y0+b,BLACK);             //6
    LCD_Fast_DrawPoint(x0-a,y0+b,BLACK);             //1
    LCD_Fast_DrawPoint(x0-b,y0+a,BLACK);
    LCD_Fast_DrawPoint(x0-a,y0-b,BLACK);             //2
    LCD_Fast_DrawPoint(x0-b,y0-a,BLACK);             //7
    a++;
    //使用Bresenham算法画圆
    if(di<0)di +=4*a+6;
    else
...
void LCD_ShowChar(uint16_t x,uint16_t y,u16 BC,u16 FC,uint8_t num,uint8_t size,uint8_t mode)
{
  uint8_t temp,t1,t;
  uint16_t y0=y;
...
void LCD_ShowString(uint16_t x,uint16_t y,uint16_t width,uint16_t height,uint8_t size,uint8_t *p)
{
  ...
  while((*p<='~')&&(*p>=' '))//判断是不是非法字符!
  {
    ...
    LCD_ShowChar(x,y,WHITE,BLACK,*p,size,0);
    x+=size/2;
    p++;
  }
}
		...
		for(i=0;i<width/8;i++)//列
		{
				for(k=0;k<8;k++)
				{
					...
					if(*p & (0x80>>k)) //从最高开描点，只须判断一位
						LCD_Fast_DrawPoint(x,y,f_color);//描字符颜色
					else
						LCD_Fast_DrawPoint(x,y,b_color);//描背景颜色 
...
#include "usart1.h"
//显示汉字和字符
void LCD_SHOW(u16 x,u16 y,u16 BC, u16 FC,u8 *buff)
{
	u16 x0=x;
	u8 qu = 0;//区码
	u8 wei = 0;//位码
  u32 offset = 0;//偏移量
	...
	while(*buff != '\0')
	{
		if(*buff > 0xA0)//汉字
		{
			qu = *buff-0xA0;
			wei = *(buff+1)-0xA0;
			offset = Load_BASEADDR+ ((qu-1)*94+(wei-1))*32;
			//一个字的字模 32字节  16 区：前面有15个区 每区94个字   16位前面有15个字  
			
			sFLASH_ReadBuffer(str,offset,32);//读取字模
			LCD_ShowxXx(x,y,16,16,BC,FC,str);//显示
			x+=16;//每个汉字偏移位置  水平显示
			buff+=2;
		...
		{
			LCD_ShowChar(x,y,BC,FC,*buff,16,0);
			x+=8;
			buff+=1;
			if(x> lcddev.width-8)
			{
...
***** 功能：    绘制大小为16×8，16x16的中英文字符串
***** 参数：    x：横坐标   y：纵坐标     color文字颜色  
   	            backColor  文字背景颜色  ch：字模数组
***** 返回值：	无
***** 注意 ：   字库在zi.c文件当中  汉字 2字节  ASCII 1字节
****************************************************************************/
	...
	while (*chs != '\0')
	{
		if (*chs > 0xa0)//汉字内码都是大于0xa0
		{
			...
			{	
				...
				{
					LCD_ShowxXx(x,y,16,16,backColor,color,&hz[32*(i)]);
					x +=16;
					break;
				}
			}
		...
		{	
			if(x>239-8) //如果到了屏幕的边缘，就换行
			{
				x=x0;
				y=y+16;
			}
			LCD_ShowChar(x,y,backColor,color,*chs,16,0);
			x+=8;
			chs++;
		}	 
	}
